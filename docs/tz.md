...
**Шесть выбросов по выручке к TVL — закрыто на шаге 06.** CC (9647%),
QUAI (9605%), LMTS (3653%), KNC (865%), OPN (493%), EDEL (541%).
`revenuePerTvlPct` выше `DISCOVERY.maxRevenuePerTvlPct` не участвует в
перцентилях сектора: число в кандидате не тронуто, перцентиль `null`, тикеры в
`warnings` ответа `GET /universe/alpha`.
