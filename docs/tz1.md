...
- один admin-ключ (`X-Admin-Key`, ШАГ 2 `docs/tz-crypto-dashboard.md`) защищает
  общие мутации; персональный многопользовательский `ActiveFilterState` для
  разных посетителей сайта всё ещё не реализован.
